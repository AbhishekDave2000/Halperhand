$(document).ready(function () {
    ...
    $('#submit1').unbind().click(function (e) {
        if (signUpValidation() != false) {
            var data = $('.customer-register-form').serialize();
            $.ajax({
                url: url + '?controller=Authentication&function=Signup',
                type: 'post',
                data: data,
                success: function (result) {
                    $(".error").remove();
                    if (result == 1) {
                        window.location.href = url + '?controller=Default&function=homepage';
                    } else if (result == 2) {
                        $('.submit-btn-main-div').before('<span class="error">This Email Already exists. Try again with diffterent one!</span>');
                    } else if (result == 0) {
                        $('.submit-btn-main-div').before('<span class="error">Something went wrong try again!</span>');
                    } else {
                        $('.submit-btn-main-div').before('<span class="error">' + result + '!</span>');
                    }
                }
            });
            e.preventDefault();
    ...
    $('#submit2').unbind().click(function (e) {
        if (signUpValidation() != false) {
            var data = $('.pro-register-form').serialize();
            $.ajax({
                url: url + '?controller=Authentication&function=Signup',
                type: 'post',
                data: data,
                success: function (result) {
                    ...
                    if (result == 1) {
                        ...
                        $('.get-started-btn-main-div').before('<span class="error">This Email Already exists. Try again with diffterent one!</span>');
                    } else if (result == 0) {
                        $('.get-started-btn-main-div').before('<span class="error">Something went wrong try again!</span>');
                    } else {
                        $('.get-started-btn-main-div').before('<span class="error">' + result + '!</span>');
                    }
                }
            });
        }
